...
document.addEventListener(
  ...
  async (event: any) => {
    if (event.target?.readyState === "complete") {
      ...
      const credentials: { username: string; password: string } | undefined = await new Promise((resolve) => {
      ...
      for (let i = 0; i < forms.length; i++) {
        ...
        for (let j = 0; j < inputs.length; j++) {
          // Autofill
          if (credentials && inputs[j].getAttribute("name") === "username") {
            inputs[j].value = credentials.username;
          } else if (credentials && (inputs[j].getAttribute("name") === "password" || inputs[j].type === "password")) {
            inputs[j].value = credentials.password;
          }
